...
{
    public:
        RingBuffer(initializer_list<T> elements);
        ~RingBuffer();
        void add(T element);
        T get(int idx) const;
...
template <class T>
RingBuffer<T>::RingBuffer(initializer_list<T> elements) :
    _headPtr(0), _isFull(true)
{
    _currNumElements = elements.size();
    _maxNumElements = elements.size();

    _internalArray = new T[_maxNumElements];
    int count = 0;
    for(auto elem: elements)
    {
        _internalArray[count] = elem;
        count++;
    }
}
